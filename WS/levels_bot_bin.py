...
class Strategy:
    def __init__(self,
        ...
        params = pika.URLParameters('amqp://guest:guest@51.15.55.46:5672/%2f')
        ...
        self.channels = {'kline_1m', "depth5"}
        self.markets = [self.symbols[0].lower()]
        self.bot_ping = True
        self.status = False
    ...
    def open_hadg_buy(self, my_price):
        ...
        my_message = f"BUY {self.hadg_symbol}\nprice: {current_price} | amount: {new_amount_future}\n (TRIANGLE)"
    ...
    def open_hadg_sell(self, my_price):
        ...
        current_price = normal_round(float(my_price), self.n_rools[self.hadg_symbol]['price'])
        my_message = f"SELL {self.hadg_symbol}\nprice: {current_price} | amount: {new_amount_future}\n (TRIANGLE)"
        self.BrockerManager.telega_msg(my_message)
        print(my_message)
    ...
    def exit_from_buy_hadg(self, my_price, status):
        ...
        my_message = f"{status} \n exit {self.hadg_symbol} (BUY)\n" \
                     f"price: {float(my_price)} \n" \
                     f"profit: {round(profit, 2)} USD / {round(percent, 2)} %"
    ...
    def on_change_price(self, data):

        sym = data["symbol"].upper()
        if data['status'] != 'running':
            msg = f"(TRIANGLE) Server not running (Symbol: {sym})"
    ...
    # def close_symbol2(self):
    #     print("closing TRIANGLE")
    #     ordersf = self.BrockerManager.my_futures_account_info()
    #     if ordersf:
    #         for i in ordersf['positions']:
    #             if i['symbol'] == self.hadg_symbol:
    #                 if float(i['entryPrice']) != 0:
    #                     if float(i['positionAmt']) < 0:
    #
    #                         close_amount = normal_round(abs(float(i['positionAmt'])),
    #                                                     self.n_rools_futures[self.hadg_symbol]["decimals"])
    #                         if self.n_rools_futures[self.hadg_symbol]["decimals"] == 0:
    #                             close_amount = int(float(close_amount))
    #                         self.BrockerManager.close_buy_market_future_order(symbol=i['symbol'],
    #                                                                           amount=close_amount)
    #
    #                     else:
    #                         close_amount = normal_round(abs(float(i['positionAmt'])),
    ...
    #                         if self.n_rools_futures[self.hadg_symbol]["decimals"] == 0:
    #                             close_amount = int(float(close_amount))
    #                         self.BrockerManager.close_sell_market_future_order(symbol=i['symbol'],
    #                                                                            amount=close_amount)
    #                     time.sleep(1)
    #
    #     print("closing TRIANGLE")
    ...
    def run(self):
        ...
        binance_websocket_api_manager = BinanceWebSocketApiManager(exchange="binance.com-futures",
                                                                   ...
                                                                   stream_buffer_maxlen=3)
        multi_stream_id = binance_websocket_api_manager.create_stream(self.channels,
                                                                      self.markets,
        ...
        while self.bot_ping:
            if self.status:
                ...
                print(f"BINANCE RESTART at {datetime.now().strftime('%H:%M:%S')}\n (TRIANGLE) {self.hadg_symbol} ")
            else:
                try:
                    if binance_websocket_api_manager.is_manager_stopping():
                        print("is_manager_stopping")
                        self.status = True
                    stream_buffer = binance_websocket_api_manager.pop_stream_data_from_stream_buffer(mode='LIFO')
                    data_status = binance_websocket_api_manager.get_stream_info(multi_stream_id)

                    my_stat = 'running'

                    if data_status['status'] != 'starting':
                        if data_status['status'] != 'running' or data_status['has_stopped']:
                            my_stat = 'stopped'
                            self.status = True
                            print(f"data_status: \n {data_status}")
                    ...
                    if stream_buffer:
                        data = {}
                        if 'stream' in stream_buffer.keys():
                            result = stream_buffer['stream'].split("@")
                            data["data"] = stream_buffer
                            data["status"] = my_stat
                            data["symbol"] = result[0]
                            data["channel"] = f"{result[1]}f"
                            self.on_change_price(data)
                    ...
                    print(f"(TRIANGLE) {self.hadg_symbol} ERROR KeyError: \n {exc}")
                except Exception as exc:
                    print(f"(TRIANGLE) {self.hadg_symbol} ERROR Exception: \n {exc}")
                    time.sleep(0.1)
